$(document).ready(function(){
        ...
        success: function(data) {
            ...
            for(var i = 0; i < repos.length; i++){
                html += '<tr><td><a href="' + repos[i].html_url + '" target="_blank" rel="nofollow">' + repos[i].name + '</a></td>' +
                    '<td><a href="' + clone_repo_url + '?repo_url=' + repos[i].ssh_url + '&repo_name=' + repos[i].name + '" data-method="post" data-remote="true" class="push-to-cibox">clone & push to cibox</a></td></tr>';
            }
    ...
    $('#repos_list').on('click', '.push-to-cibox', function(){
        $('#load-repos-indicator').show();
    });
});
